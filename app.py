...
st.title("📊 منصة التحليل المالي وإدارة المحافظ الاستثمارية")
st.markdown("---")

# Clean Sidebar Navigation
st.sidebar.title("🎛️ لوحة التحكم الرئيسية")
page = st.sidebar.selectbox("اختر القسم المطلوب:", [
    "📊 مسح السوق والأسعار اللحظية",
    "🎯 تحليل الفرص ونقاط الدخول",
...
if "مسح السوق والأسعار اللحظية" in page:
...
elif "تحليل الفرص ونقاط الدخول" in page:
    st.header("🎯 تحليل الفرص اليومية وتحديد المستويات السعرية")
    st.info("تحديد مستويات الدعم، المقاومة، وأسعار الدخول الاستراتيجية بناءً على البيانات التاريخية.")
    
    spec_ticker = st.text_input("أدخل رمز السهم المراد تحليله (مثال: COMI.CA)", value="COMI.CA")
        ...
        with st.spinner("جاري معالجة بيانات السهم واستخراج المستويات..."):
            try:
                ...
                if not hist_data.empty:
                    ...
                    target_1 = p_curr * 1.025
                    target_2 = p_high
                    
                    st.success(f"تقرير التحليل الفني للسهم: {spec_ticker}")
                    
                    sc1, sc2, sc3, sc4 = st.columns(4)
                    sc1.metric("سعر الدخول المقترح", f"{p_curr:.2f} ج.م")
                    sc2.metric("مستوى وقف الخسارة", f"{stop_loss:.2f} ج.م")
                    sc3.metric("الهدف السعري الأول", f"{target_1:.2f} ج.م")
                    sc4.metric("مستوى المقاومة", f"{target_2:.2f} ج.م")
                    
                    st.markdown("---")
                    st.subheader("ملاحظات تنفيذية:")
                    st.markdown(f"""
                    * **نطاق التداول:** السهم يتحرك ضمن نطاق فني مستقر يتيح بناء مراكز تدريجية.
                    * **التنفيذ:** يفضل الشراء حول مستوى **{entry_price:.2f} ج.م**.
                    * **إدارة المخاطر:** ضرورة الالتزام بمستوى وقف الخسارة الموضح لحماية رأس المال.
                    """)
                else:
